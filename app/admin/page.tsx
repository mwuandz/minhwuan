            ...
            {tab === "products" && (
              <section className="admin-grid">
                <form className="card form admin-section" onSubmit={saveProduct}>
                  ...
                  <input placeholder="Ảnh/Icon sản phẩm, ví dụ 🤖 🎬 🎧" value={productForm.image || ""} onChange={(e) => setProductForm({ ...productForm, image: e.target.value })} />
                  ...
                  <div className="product-admin-list">
                    {products.map((product) => (
                      <article className="product-admin-card" key={product.id}>
                        <span className="admin-icon">{product.image || "💎"}</span>
                  ...
                  <div className="product-admin-list">
                    {vouchers.map((voucher) => (
                      <article className="product-admin-card" key={voucher.code}>
                        ...
                        <div><b>{voucher.code}</b><small>{voucher.description || "-"} · Đã dùng {voucher.usedCount || 0}{voucher.usageLimit ? `/${voucher.usageLimit}` : ""}</small></div>
