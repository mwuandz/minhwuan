...
export default function HomePage() {
  ...
  return (
    <main className={settings.themeMode === "dark" ? "theme-dark" : "theme-light"} style={{ ["--accent" as string]: settings.accentColor }}>
      ...
      {settings.flashSaleEnabled !== false && flashProducts.length > 0 && !flashExpired && (
          ...
          <div className="container flash-sale-inner">
            ...
            <div className="flash-products">
              {flashProducts.slice(0, 4).map((product) => (
                <button key={product.id} className="flash-product" onClick={() => { addToCart(product.id); setCartOpen(true); }}>
            ...
            <div className="flash-action">
              ...
              <b>{flashRemaining}</b>
              <a href="#products">{settings.flashSaleCta || "Mua ngay"}</a>
            </div>
          </div>
        </section>
      )}
