...
export default function HomePage() {
  ...
  async function applyVoucher() {
    ...
    const res = await fetch("/api/vouchers/validate", {
      ...
      body: JSON.stringify({ code: voucherInput, amount: cartTotal })
    });
            ...
            <div className="product-grid">
              {filteredProducts.map((product) => (
                <article className="product-card product-card-animated" key={product.id}>
                  <div className="product-image"><span>{product.image || "💎"}</span>{product.badge && <b>{product.badge}</b>}</div>
        ...
        <section id="checkout" className="checkout-card">
          ...
          <form className="form" action={createOrder}>
            <div className="cart-summary">
              ...
              {voucher && <div className={voucher.valid ? "notice success compact" : "notice error compact"}>{voucher.message}{voucher.valid ? ` · Giảm ${formatMoney(voucher.discountAmount)}` : ""}</div>}
