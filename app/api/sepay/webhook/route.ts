...
function normalizeOrderId(raw: string) {
  ...
  if (/^MW-\d+$/.test(cleaned)) return cleaned;
  if (/^MW\d+$/.test(cleaned)) return cleaned.replace(/^MW/, "MW-");
...
function findOrderIdFromText(text: string) {
  ...
  const matches = upperText.match(/MW-?\d+/g) || [];

  // SePay đôi khi trả field `code` bị rút ngắn, còn `content` mới là mã đơn đầy đủ.
  // Vì vậy lấy mã dài nhất để tránh tìm nhầm MW-1783688142 thay vì MW-1783688142976.
...
function getTransactionContent(body: Record<string, unknown>) {
  return String(
    [
      // Ưu tiên nội dung chuyển khoản đầy đủ trước, vì `code` của SePay có thể bị cắt ngắn.
      body.content,
      body.description,
