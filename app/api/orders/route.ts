...
import { getProduct, saveOrder, validateVoucher } from "../../../lib/redis";
...
export async function POST(req: Request) {
  try {
    ...
    const voucherResult = voucherCode ? await validateVoucher(voucherCode, originalAmount) : null;
    ...
    try {
      await sendTelegramMessage(
        ...
        `${discountAmount > 0 ? `Voucher: ${voucherCode} -${discountAmount.toLocaleString("vi-VN")}đ\n` : ""}` +
