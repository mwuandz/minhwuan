...
import { Order, Product, SiteSettings, Voucher, VoucherValidation } from "./types";
...
const PRODUCT_CATALOG_VERSION = "2026-07-08-app-price-list";
...
export function calculateVoucherDiscount(voucher: Voucher, amount: number): VoucherValidation {
  const now = Date.now();
  if (!voucher || voucher.active === false) return { valid: false, discountAmount: 0, message: "Voucher không hoạt động" };
  if (voucher.expiresAt && new Date(voucher.expiresAt).getTime() < now) return { valid: false, discountAmount: 0, message: "Voucher đã hết hạn" };
  if (voucher.minOrderAmount && amount < voucher.minOrderAmount) return { valid: false, discountAmount: 0, message: `Đơn tối thiểu ${voucher.minOrderAmount.toLocaleString("vi-VN")}đ` };
  if (voucher.usageLimit && (voucher.usedCount || 0) >= voucher.usageLimit) return { valid: false, discountAmount: 0, message: "Voucher đã hết lượt sử dụng" };

  const rawDiscount = voucher.type === "percent" ? Math.floor((amount * voucher.value) / 100) : voucher.value;
  const capped = voucher.maxDiscount ? Math.min(rawDiscount, voucher.maxDiscount) : rawDiscount;
  const discountAmount = Math.max(0, Math.min(amount, capped));

  if (discountAmount <= 0) return { valid: false, discountAmount: 0, message: "Voucher chưa có giá trị giảm hợp lệ" };
  return { valid: true, voucher, discountAmount, message: `Đã áp dụng ${voucher.code}` };
}

export async function validateVoucher(code: string, amount: number): Promise<VoucherValidation> {
  const voucher = await getVoucher(code);
  if (!voucher) return { valid: false, discountAmount: 0, message: "Không tìm thấy voucher" };
  return calculateVoucherDiscount(voucher, amount);
}
